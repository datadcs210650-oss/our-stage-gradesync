...
function updateStatus(){const p=document.querySelector('#pending-state');if(p)p.textContent=`${pendingScores().length} chờ đồng bộ`;updateEmergency(store.get().system,currentUser)}
...
const routeEvents={dashboard:new Set(['groups','candidates','scores','verifications','presence','system','user','selection','state']),groups:new Set(['groups','selection']),candidates:new Set(['groups','candidates','selection']),queue:new Set(['groups','candidates','selection','system']),live:new Set(['groups','candidates','scores','verifications','presence','graders','system']),grading:new Set(['groups','candidates','scores','verifications','graders','system','user','selection']),analytics:new Set(['groups','candidates','scores','selection']),reports:new Set(['groups','candidates','scores','selection','system']),graders:new Set(['groups','user']),backup:new Set([]),audit:new Set([])};
store.subscribe((_state,type)=>{if(routeEvents[route]?.has(type)||type==='state')scheduleRender()});
window.addEventListener('online',async()=>{if(currentUser?.role==='Grader'){const r=await flushScoreQueue(currentUser);if(r.done)toast(`Đã đồng bộ ${r.done} bảng điểm ngoại tuyến`);if(r.conflicts)toast(`${r.conflicts} bảng điểm ngoại tuyến bị xung đột, chưa ghi đè dữ liệu mới trên máy chủ.`,'error');updateStatus()}});
window.addEventListener('offline',()=>toast('Mất kết nối. Điểm mới sẽ được lưu vào hàng chờ ngoại tuyến.','error'));
